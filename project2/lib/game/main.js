...
MyGame = ig.Game.extend({
    ...
    update: function() {
    	...
    	if( player ) {
    		...
    		this.screen.y = ig.system.height/2 - 120;
			if(player.accel.x != 0 && this.instructText)
                this.instructText = null;
    	}
		var playert = this.getEntitiesByType( EntityPlayert )[0];
    	if( playert ) {
    		this.screen.x = playert.pos.x - ig.system.width/2;
    		this.screen.y = playert.pos.y - ig.system.height/2 ;
    	}
		var playerw = this.getEntitiesByType( EntityPlayerw )[0];
    	if( playerw ) {
    		this.screen.x = playerw.pos.x - ig.system.width/2;
    		this.screen.y = playerw.pos.y - ig.system.height/2;
    	}
	...
	draw: function() {
        ...
        if(this.instructText){
            ...
            this.instructText.draw( 'The castle has been overrun!', x, y - 10, ig.Font.ALIGN.CENTER );
			this.instructText.draw( 'I have given you a special weapon,', x, y, ig.Font.ALIGN.CENTER );
			this.instructText.draw( 'the Sword Gun! Press C to attack,', x, y + 10 , ig.Font.ALIGN.CENTER );
			this.instructText.draw( 'arrow keys move left/right, X to jump,', x, y + 20, ig.Font.ALIGN.CENTER );
			this.instructText.draw( 'and TAB to switch firing mode!', x, y + 30, ig.Font.ALIGN.CENTER );
			this.instructText.draw( '  /                         ', x, y + 40, ig.Font.ALIGN.CENTER );
        }
        if(this.showStats){
        ...
        for(var i=0; i < this.lives; i++)
            this.lifeSprite.draw(((this.lifeSprite.width + 2) * i)+5 + 24, 3);
	},
...
GameOverScreen = ig.Game.extend({
    ...
    draw: function() {
        ...
        var x = ig.system.width/2;
        var y = ig.system.height/2 - 20;
        this.gameOver.draw(x - (this.gameOver.width * .5), y + 65);
...
FinishScreen = ig.Game.extend({
    instructText: new ig.Font( 'media/04b03.font.png' ),
    background: new ig.Image('media/screen-bg.png'),
    end: new ig.Image('media/end.png'),
    stats: {},
    init: function() {
        ig.input.bind( ig.KEY.SPACE, 'start');
        this.stats = ig.finalStats;
    ...
    draw: function() {
        ...
        this.end.draw(x - (this.end.width * .5), y);
        //var score = (this.stats.kills * 100) - (this.stats.deaths * 50);
        //this.instructText.draw('Total Kills: '+this.stats.kills, x, y+30, ig.Font.ALIGN.CENTER);
        //this.instructText.draw('Total Deaths: '+this.stats.deaths, x, y+40, ig.Font.ALIGN.CENTER);
        //this.instructText.draw('Score: '+score, x, y+50, ig.Font.ALIGN.CENTER);
        this.instructText.draw('Press Spacebar To Restart.', x, ig.system.height - 10, ig.Font.ALIGN.CENTER);
    }
});
